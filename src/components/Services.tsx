          ...
          {services.map((service, index) => (
            ...
            >
              <div className="flex items-start mb-4">
                ...
                <h3 id={`service-title-${index}`} className="text-xl font-semibold text-zen-forest">
                  {service.title}
                </h3>
              </div>
              
              <p className="text-zen-forest/80 mb-6 leading-relaxed">
                {service.description}
              </p>
              
              <div className="flex justify-between items-center pt-4 border-t border-zen-sand/30">
                <div className="flex items-center text-zen-forest/70">
                  <Circle className="h-4 w-4 mr-2" aria-hidden="true" />
                  <span className="text-sm">{service.duration}</span>
                </div>
                <div className="text-zen-sage font-semibold">
                  {service.price}
                </div>
              </div>
            </div>
